...
class ClickManager {
  ...
  keyUpMethod = (e) => {
    const keyName = this.getKeyName(e.key);
    this.currentClickedButtons = this.currentClickedButtons.filter(
      (key) => key !== keyName
    );
  ...
  getKeyName(key) {
    ...
    if (typeof key == "string") keyName = ALIAS[key] || key;
...
class EventsManager {
  ...
  remove() {
    this.events.forEach(([eventName, callback]) =>
      this.config.element.removeEventListener(eventName, callback)
    );
    this.events = [];
  }
}
...
class KeyboardActions {
  ...
  start() {
    ...
    this.eventsManager.addEventListener(
      "keydown",
      this.clickManager.keyDownMethod
    );
